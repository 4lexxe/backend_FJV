...
personaCtrl.createPersona = async (req, res) => {
  ...
  try {
    ...
    if (
      !personaData.nombreApellido ||
      personaData.nombreApellido.trim() === ""
    ) {
    ...
    if (
      ...
      personaData.fechaNacimiento.trim() === ""
    ) {
      return res.status(400).json({
        status: "0",
    ...
    if (
      personaData.fechaLicencia &&
      isNaN(new Date(personaData.fechaLicencia).getTime())
    ) {
      return res.status(400).json({
        status: "0",
    ...
    personaData.fechaLicenciaBaja = fechaLicenciaBaja
      .toISOString()
      .split("T")[0];
    ...
    Object.keys(personaData).forEach((key) => {
      if (
        typeof personaData[key] === "string" &&
        personaData[key].trim() === ""
      ) {
    ...
    const datosFinales = {
      ...personaData,
      idClub:
        idClub && idClub !== "" && idClub !== "null" ? parseInt(idClub) : null,
    };
...
personaCtrl.editPersona = async (req, res) => {
  ...
  try {
    ...
    try {
      if (
        personaData.hasOwnProperty("fechaLicencia") &&
        personaData.fechaLicencia
      ) {
        ...
        const fechaActual = new Date();
        personaData.estadoLicencia =
          fechaLicenciaBaja >= fechaActual ? "ACTIVO" : "INACTIVO";

        if (
          personaExistente.credenciales &&
          personaExistente.credenciales.length > 0
        ) {
      ...
      if (req.imageData && personaExistente.fotoPerfilDeleteUrl) {
        try {
          ...
          console.error(
            "Error eliminando imagen anterior de ImgBB:",
            deleteError
          );
        }
      }
    ...
    try {
      for (const persona of personas) {
        if (persona.fechaLicenciaBaja) {
          const nuevoEstado =
            persona.fechaLicenciaBaja >= fechaActual ? "ACTIVO" : "INACTIVO";
...
personaCtrl.getResumen = async (req, res) => {
  try {
    const totalFJV = await Persona.count({ where: { licenciaFEVA: null } });
    const totalFEVA = await Persona.count({
      where: { licenciaFEVA: { [Op.ne]: null } },
    });
    res.json({ fjv: totalFJV, feva: totalFEVA, total: totalFJV + totalFEVA });
  } catch (e) {
...
personaCtrl.getCantidadPorCategoria = async (req, res) => {
  try {
    const tipos = await Persona.findAll({
      attributes: ["tipo", [fn("COUNT", col("tipo")), "cantidad"]],
      group: ["tipo"],
      where: {
        tipo: {
          [require("sequelize").Op.ne]: null, // Esto asegura que solo se cuenten los tipos que no son nulos
        },
      },
    });
    res.json(tipos);
    ...
    res
      .status(500)
      .json({ error: "Error al obtener la cantidad por tipo", msg: e.message });
  }
};

personaCtrl.getCantidadPorClub = async (req, res) => {
  try {
    const afiliadosPorClub = await Persona.findAll({
      attributes: [
        [col("Persona.idClub"), "idClub"],
        [fn("COUNT", col("idPersona")), "totalAfiliados"],
      ],
      // --- ¡CAMBIO CRÍTICO AQUÍ! ---
      // Añade col('club.idClub') al GROUP BY
      group: [col("Persona.idClub"), col("club.idClub"), "club.nombre"],
      include: [{ model: Club, attributes: ["nombre"], as: "club" }],
    });

    const resultado = afiliadosPorClub.map((p) => ({
      nombreClub: p.club?.nombre || "Sin club",
      totalAfiliados: p.get("totalAfiliados"),
    }));
    ...
    res
      .status(500)
      .json({ error: "Error al obtener afiliados por club", msg: e.message });
  }
};
